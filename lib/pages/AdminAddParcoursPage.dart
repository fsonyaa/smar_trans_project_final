...
class AdminAddParcoursPage extends StatefulWidget {
  const AdminAddParcoursPage({super.key});

  @override
  State<AdminAddParcoursPage> createState() => _AdminAddParcoursPageState();
}
...
class _AdminAddParcoursPageState extends State<AdminAddParcoursPage> {
  ...
  void dispose() {
    ...
    for (final c in heureControllers) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Future<void> fetchLignes() async {
    try {
      final data = await FirestoreService.getAllLignes();
      if (mounted) {
        setState(() {
          lignes = data;
      ...
      if (mounted) setState(() => isLoadingLignes = false);
    }
  }
