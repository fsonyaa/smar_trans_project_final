...
// Charger le token depuis firebase-tools.json
...
function uploadDocument(user) {
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      path: `/v1/projects/${project}/databases/(default)/documents/${user.collection}/${user.docId}?currentDocument.exists=false`,
      method: 'PATCH',
      headers: {
    ...
    // Si exists=false échoue (parce qu'il existe déjà), on va forcer l'écriture sans condition
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          // Essayer sans condition de non-existence au cas où
          const retryOptions = {
            ...options,
          ...
          const retryReq = https.request(retryOptions, (retryRes) => {
            ...
            retryRes.on('end', () => {
              if (retryRes.statusCode >= 200 && retryRes.statusCode < 300) {
                console.log(`Successfully overwrote/wrote ${user.collection}/${user.docId}`);
                resolve();
              } else {
                console.error(`Failed to write ${user.collection}/${user.docId}:`, retryRes.statusCode, retryBody);
                reject(new Error(retryBody));
              }
            });
          });
